...
import { FaTools, FaArrowLeft, FaCompass } from 'react-icons/fa';
...
const GlobalStyle = () => (
  <style>{`
    ...
    .eq-fade-up { opacity: 0; animation: eq-fadeUp 0.7s ease-out forwards; }
    .eq-blob-a { animation: eq-drift 16s ease-in-out infinite; }
    .eq-blob-b { animation: eq-drift-rev 18s ease-in-out infinite; }
    .eq-float { animation: eq-float 4.5s ease-in-out infinite; }
    @media (prefers-reduced-motion: reduce) {
      .eq-fade-up, .eq-blob-a, .eq-blob-b, .eq-float { animation: none !important; }
    }
  `}</style>
);
...
const Enquiry = () => {
  return (
    <div
      className="min-h-screen text-stone-800 flex items-center justify-center pt-24 pb-16 px-4 relative overflow-hidden"
      style={{ background: 'linear-gradient(180deg, #FAF8FC 0%, #F1EBF8 55%, #FAF8FC 100%)' }}
    >
      <GlobalStyle />

      {/* Ambient Background Accents */}
      <div
        className="eq-blob-a absolute top-1/4 left-1/4 w-[400px] h-[400px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.18) 0%, rgba(124,58,237,0) 70%)', filter: 'blur(60px)' }}
      ></div>
      <div
        className="eq-blob-b absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(217,70,160,0.18) 0%, rgba(217,70,160,0) 70%)', filter: 'blur(60px)' }}
      ></div>

      <div className="eq-fade-up max-w-lg w-full bg-white/90 backdrop-blur-xl rounded-3xl border border-violet-100 shadow-2xl shadow-violet-200/40 p-8 sm:p-12 text-center relative z-10">

        {/* Animated Icon Container */}
        <div className="eq-float w-20 h-20 mx-auto mb-6 rounded-3xl bg-gradient-to-tr from-violet-600 to-fuchsia-500 flex items-center justify-center text-white text-3xl shadow-lg shadow-violet-500/25">
          <FaCompass className="animate-spin-slow" />
        </div>

        {/* Badge */}
        <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-violet-50 text-violet-700 text-xs font-bold uppercase tracking-wider mb-4 border border-violet-100">
          <FaTools className="text-[11px]" /> Coming Soon
        </div>

        {/* Title */}
        <h1 className="text-2xl sm:text-3xl font-extrabold text-stone-900 tracking-tight">
          under Construction: Enquiry Management
        </h1>

        {/* Description */}
        <p className="text-xs sm:text-sm text-stone-500 mt-3 leading-relaxed">
          This section or feature is currently under construction as we build an even better travel booking experience for you. Check back soon!
        </p>
        ...
        <div className="mt-8">
          <Link
            to="/"
            className="w-full py-3.5 px-6 rounded-2xl font-bold text-xs uppercase tracking-wider shadow-lg bg-gradient-to-r from-violet-600 to-fuchsia-500 hover:from-violet-700 hover:to-fuchsia-600 text-white shadow-violet-500/25 transition-all inline-flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99]"
          >
